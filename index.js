...
function findIndexByProperty(arr, name) {
  ...
  // Використовуємо метод findIndex для пошуку індексу об'єкта, властивість 'name' якого співпадає з заданим іменем. // Повертаємо знайдений індекс або -1, якщо об'єкт не знайдено.
}
...
function findLastEvenNumber(arr) {
  ...
  if (Array.isArray(arr)) {
    if (
      (lastEvenNumber = arr.findLast((elem) => {
        return elem % 2 === 0;
      }))
    ) {
      return lastEvenNumber;
...
function getPositiveArrayValues(arr) {
  let res;
  if (Array.isArray(arr)) {
    res = Array.from(arr.values());
    return res.filter((elem) => elem > 0);
  } else return `Вхідний параметр має бути масивом`;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function removeAndDouble(arr, startIdx) {
  let res;
  if (Array.isArray(arr)) {
    res = arr.splice(0, startIdx);
    return res.map((elem) => elem * 2);
  } else return `Вхідний параметр має бути масивом`;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function flattenSquareAndFilter(arr, threshold) {
  let res;
  if (Array.isArray(arr)) {
    res = arr.flat().map((elem) => elem ** 2);
    return res.filter((elem) => elem > threshold);
  } else return "Вхідний параметр має бути масивом";
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function getLastElementsSumAndPrint(arr, n) {
  let res;
  if (Array.isArray(arr)) {
    let arrNew = arr.slice(-n);
    res = arrNew.reduceRight((sum, elem) => elem + sum);
    arrNew.forEach((elem) => {
      return console.log(elem);
    });
    return res;
...
function expandArrayByN(arr, n) {
  if (Array.isArray(arr) && typeof n === "number") {
    const result = arr.flatMap((elem) => {
      return elem === elem ? [elem, elem, elem] : 0;
    });
    return result;
...
function findLongestWord(arr) {
  if (Array.isArray(arr)) {
    let longestWord = arr.reduce((maxLenghtWord, elem) => {
      return elem.length > maxLenghtWord.length ? elem : maxLenghtWord;
    });
    return longestWord;
  } else return " ";
...
function findDuplicateElements(arr) {
  if (Array.isArray(arr)) {
    let duplicateElements = arr.filter((elem, index) => {
      return arr.indexOf(elem) !== index; //повертає неунікальні елементи
    });
...
function capitalizeWords(arr) {
  if (Array.isArray(arr)) {
    let capitalizedArray = arr.map((elem) => {
...
function calculateTotalPrice(arr) {
  if (Array.isArray(arr)) {
    let totalPrice = arr.reduce((sum, price) => {
...
function findLastNegativeNumberIndex(arr) {
  if (Array.isArray(arr)) {
    let lastIndex = arr.findLastIndex((elem, index) => {
